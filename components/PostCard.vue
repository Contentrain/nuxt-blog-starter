<template>
  <div v-if="post" class="post-card my-2 px-2 w-full md:w-1/2 lg:my-6 lg:px-6 lg:w-1/3">
    ...
    <div class="post-card__header text-gray-500 flex items-center py-2 mb-2">
      <span class="w-6 h-6"
        >
        <img
      ...
      <i class="ri-checkbox-blank-circle-fill text-gray-300 text-xs"></i>
      <span class="text-xs mx-2">{{new Date(post.createdAt).toLocaleDateString()}}</span>
    </div>
    <div class="post-card_title">
      <h2 class="text-xl font-semibold mb-2 overflow-hidden overflow-ellipsis h-7">{{ post.title }}</h2>
    </div>
    <div class="post-card_description">
      <p
        class="text-base text-gray-500 mb-2 overflow-hidden overflow-ellipsis h-12"
      >
        {{ post.description }}
      </p>
    </div>
